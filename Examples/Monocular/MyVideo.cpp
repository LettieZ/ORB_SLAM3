...
// 该文件将打开给定的视频文件，并将图像传递给ORB-SLAM进行定位
...
string vocFile = "/home/z/ORB_SLAM3/Vocabulary/ORBvoc.txt";
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        cv::resize(frame, frame_resized, cv::Size(640,480)); //320,240  640,480

        auto now = chrono::system_clock::now();
        auto timestamp = chrono::duration_cast<chrono::milliseconds>(now - start);
	//std::cout << "timestamp:" << double(timestamp.count())/1000.0 << std::endl;
